  ...
  >
    <template #title="{ title, url,id }">
      <div class="bookmark-text">
        <span v-if="title.indexOf(searchValue) > -1" :title="title" class="bookmark-text-link" @click="linkTo(url)">
          {{ title.substr(0, title.indexOf(searchValue)) }}
          <span style="color: #f50">{{ searchValue }}</span>
          {{ title.substr(title.indexOf(searchValue) + searchValue.length) }}
        </span>
        <span v-else class="bookmark-text-link" :title="title" @click="linkTo(url)">{{ title }}</span>
        <div class="bookmark-text-btns" v-if="url">
          <a-button type="link" size="small" @click="handleEdit(id,title)">edit</a-button>
          <a-divider type="vertical" />
          <a-button danger type="link" size="small" @click="handleDelete(id)">delete</a-button>
        </div>
      </div>
    </template>
  </a-tree>
